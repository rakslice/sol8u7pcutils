...
def parse_args():
	...
	parser.add_option("--allow-makefile",
		help="Allow makefile changes",
		default=False,
		action="store_true",
		)
...
def text_within(s, prefix, suffix):
...
def toss_patch(patch_filename, options):
	...
	with open(output_filename, "w") as output_handle:
		for filename, section_lines in sections(patch_filename):
			...
			if filename in skip_filenames:
				continue
			if filename_proper.startswith("Makefile") and not options.allow_makefile:
				continue
...
def main():
	...
	try:
		toss_patch(patch_filename, options)
	except IOError, e:
